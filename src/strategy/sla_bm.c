...
int LogIn_SlaBm(long despId, SSDBufTag tag, unsigned flag)
{
    ...
    if ((flag & SSD_BUF_DIRTY) != 0){
        ...
        myZone->cur_pagecnt_dirty++;
    } else{
        myZone->pagecnt_clean++;
        myZone->cur_pagecnt_clean++;
    }

    // blkcnt++;
    // if(blkcnt == (NBLOCK_SSD_CACHE / 10) ){
    //     pause_and_caculate_weight_sizedivhot();
    // }
    return 1;
}

int LogOut_SlaBm(long *out_despid_array, int max_n_batch)
{
...
int Hit_SlaBm(long despId, unsigned flag)
{
    Dscptr *myDesp = GlobalDespArray + despId;
